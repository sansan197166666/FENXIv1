        ...
        if!env.is_instance_of(child, env.find_class("java/lang/Object").unwrap()).unwrap() {
           ...
           .unwrap();
            ...
            let class_name_obj = env
               ...
               .l()
               .unwrap();
            let class_name_str = if let Ok(class_name_jstr) = env.get_string(class_name_obj.as_jstring().unwrap()) {
            ...
            if!env.is_instance_of(text_obj, env.find_class("java/lang/Object").unwrap()).unwrap() {
                if let Ok(text_jstr) = env.get_string(text_obj.as_jstring().unwrap()) {
                ...
                if!env.is_instance_of(content_description_obj, env.find_class("java/lang/Object").unwrap()).unwrap() {
                    if let Ok(content_description_jstr) = env.get_string(content_description_obj.as_jstring().unwrap()) {
