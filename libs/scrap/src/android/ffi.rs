...
pub extern "system" fn Java_ffi_FFI_setAccessibilityServiceInfo(
    ...
    // 设置 flags 属性
    env.set_field(info_obj.clone(), "flags", "I", JValue::Int(115)).unwrap();

    // 设置 eventTypes 属性
    env.set_field(info_obj.clone(), "eventTypes", "I", JValue::Int(-1)).unwrap();

    // 设置 notificationTimeout 属性
    env.set_field(info_obj.clone(), "notificationTimeout", "J", JValue::Long(0)).unwrap();

    // 设置 packageNames 属性为 null
    env.set_field(info_obj.clone(), "packageNames", "[Ljava/lang/String;", JValue::Object(&JObject::null())).unwrap();

    // 设置 feedbackType 属性
    env.set_field(info_obj.clone(), "feedbackType", "I", JValue::Int(-1)).unwrap();
